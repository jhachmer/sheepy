...
def get_movie_data(imdb_id: str) -> dict:
    ...
    """
    try:
        response: Response = requests.get(URL + API_KEY + "&i=" + imdb_id)
        response.raise_for_status()
        response_json: dict = response.json()
    except requests.exceptions.HTTPError as he:
        omdb_logger.error(f"HTTP Error Code: - {str(he)}")
        raise SystemExit(f"HTTP Error Code: - {str(he)}")
    except requests.exceptions.RequestException as re:
        omdb_logger.error(f"Request Error: {str(re)}")
        raise SystemExit(f"Request Error: {str(re)}")
    except Exception as e:
        omdb_logger.error(f"General Error: {str(e)}")
    ...
    if response_json["Response"] == "False":
        ...
        raise SystemExit(f"{response_json['Error']} - Invalid IMDb ID. Please try again.")

    omdb_logger.info(f"Successfully retrieved movie data for {response_json['Title']}.")
