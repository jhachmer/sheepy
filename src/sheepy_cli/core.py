...
def get_spreadsheet_and_sheet(sh_id: str = SPREADSHEET_ID) -> tuple:
    """Returns Spreadsheet and Sheet to be written in
    If no Sheet exists, one is going to be created.

    Args:
        sh_id (str, optional): Google Spreadsheet ID. Defaults to SPREADSHEET_ID.

    Raises:
        SystemExit: If unable to create new sheet.

    Returns:
        tuple: Spreadsheet [0] and Sheet [1] Objects
    """
    try:
        ss = (
...
def find_free_row(sheet: ezsheets.Sheet) -> int:
    ...
    Returns:
        int: The number of the first free row in the Sheet.
    """
